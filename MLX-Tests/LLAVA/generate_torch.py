...
from llava_torch import LlavaModel

# Same argparse and image loading functions (no change needed)
def parse_arguments():
...
def prepare_inputs(processor, image, prompt):
    ...
    inputs = processor(prompt, image, return_tensors="pt")  # 'pt' for PyTorch tensors
...
def generate_text(input_ids, pixel_values, model, processor, max_tokens, temperature):
    input_ids, pixel_values = input_ids.cuda(), pixel_values.cuda()  # Move data to GPU
    model.cuda()  # Move model to GPU

    with torch.no_grad():  # Disable gradient calculation for inference
        outputs = model(input_ids, pixel_values)
        logits = outputs.logits[:, -1, :]
        y = sample(logits, temperature=temperature).squeeze()

    tokens = [y.item()]

    for _ in range(max_tokens - 1):
        with torch.no_grad():
            outputs = model.language_model(y.unsqueeze(0), cache=outputs.cache)
            logits = outputs.logits[:, -1, :]
            y = sample(logits, temperature).squeeze()
            token = y.item()
